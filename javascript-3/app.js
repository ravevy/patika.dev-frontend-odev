...
const buttonNames = ["All"]
menu.map((item)=>{if(!buttonNames.includes(item.category)){buttonNames.push(item.category)}})
button.innerHTML = buttonNames.map((item)=>{return `<button class="btn btn-outline-dark btn-item" id="${item}">${item}</button>`}).join("")
...
else if(buttonNames.filter(item => item == category)){
